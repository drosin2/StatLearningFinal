...
```{r Lineary regression model for longer stay (on training)}

```


#Boxplot to pull out outliers
#Fit a LR model on Short term vs long term stay
...
ggplot(data = data, aes(x = bmi,
                        y = icu_los_day)) +
  geom_point() + 
  geom_smooth(method = "lm", se = FALSE) # add line
```

```{r splitting testing/training}
n <- nrow(data)
set.seed(1)
...
```{r fitting on train set}
fit <- lm(icu_los_day ~ bmi + age, data=test_data)
summary(fit) # show results
```


```{r making preds on test set}
pred_stay <- predict(fit, newdata = test_data)
```
The predictions are conservative in nature (about all the same), so how do we predict those long stays???

```{r computing rmse}
mean( (test_data$icu_los_day - pred_stay)^2 ) # test MSE
sqrt( mean( (test_data$icu_los_day - pred_stay)^2 ) ) # test RMSE
```
are these vals significant?
