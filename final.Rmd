...
#Mean / Mode table
```{r getting the mean and the mode of all columns}
meanModeTable <- data
...
train_indices2 <- sample(1:n2, size = floor(0.8*n2)) 
train_long <- long_stay %>%
  slice(train_indices2)
test_long <- long_stay %>%
  slice(-train_indices2)
...
m <- lm(formula = icu_los_day ~ ., data = subset(train_short,
                                                 select = -sepsis_flg)) 
...
m2 <- lm(formula = icu_los_day ~ ., data = subset(train_long,
                                                 select = -sepsis_flg)) 
#making predictions on the test data
...
n3 <- nrow(data)
# Common to use 70% or 80% of observations for training set
train_indices3 <- sample(1:n3, size = floor(0.8*n3)) 
train_compare <- data %>%
...
m3 <- lm(formula = icu_los_day ~ ., data = subset(test_compare,
                                                 select = -sepsis_flg))
pred_all_stay <- predict(m3, newdata = test_compare)
...
```
Across all predictions, support vector regression does better than linear regression on our dataset, reflecting the dimensionality of ICU patients
...
We can see the advantages of fitting support vector machines over linear regression models
...
```{r}
#rf.data<- read_csv("/Users/dougrosin/Downloads/mortalitydata.csv") %>% select(-X1)
rf.old<- data %>%
  filter(age> median(data$age))
...
n <- nrow(rf.old)
# Common to use 70% or 80% of observations for training set
train.indices.rf <- sample(1:n, size = floor(0.7*n)) 
train.rf <- rf.old %>%
...
glm.old <- glm(formula = censor_flg ~., 
                   data = subset(train.rf, select = -sepsis_flg),
                   family = binomial)
...
```{r predicting w test data and fitted forest}
pred_prob<- predict(glm.old, newdata = test.rf, type = "response")
...
```{r}
#Use the ideal decision threshold from the Youden Index calculation
pred_class_labels <- ifelse(pred_prob > 0.4749, "Yes", "No") # use the ideal threshold
#make a confusion matrix
conf_mtx <- table(pred_class_labels, test.rf$censor_flg)
conf_mtx
...
```{r predicting w test data and fitted forest}
pred.old <- predict(rf.old.model, newdata = test.rf, type = "response")
...
#make a confusion matrix
conf_mtx <- table(pred.old, test.rf$censor_flg)
conf_mtx
...
```{r}
#Use the ideal decision threshold from the Youden Index calculation
pred_class_labels <- ifelse(pred.prob.young.glm > 0.4236, "Yes", "No") # use the ideal threshold
#make a confusion matrix
